...
export class ExpressServer {
    ...
    private configureApiEndpoints(server: Express): void {
        // Person
        server.get('/api/student/:id', noCacheMiddleware, this.personEndpoints.getStudent);
        server.get('/api/student', noCacheMiddleware, this.personEndpoints.getAllStudents);
        server.post('/api/student', noCacheMiddleware, this.personEndpoints.postStudent);

        // Group
        server.get('/api/group/:name', this.groupEndpoints.getGroup);
        server.get('/api/group/:name/students', this.groupEndpoints.getStudentsByGroup);
        server.get('/api/group', this.groupEndpoints.getAllGroups);
        server.post('/api/group', this.groupEndpoints.postGroup);

        // Document
        server.post('/api/document', this.documentEndpoints.postLinkDocumentToGroups);
        server.get('/api/document/:documentRefId', this.documentEndpoints.getDocument);
        server.get('/api/document', this.documentEndpoints.getAllDocuments);
        server.delete('/api/document/deleteDocument/:documentRefId', this.documentEndpoints.deleteDocument);
    }
}
