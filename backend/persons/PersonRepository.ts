...
export class PersonRepository {
    ...
    async getPersonById(id: string): Promise<Person | null> {
        const result = await this.databaseConnection.getCollectionEntries(this.collectionName, { id: id });
        if (result.length >= 1) {
            return result[0];
        }
        return null;
    ...
    async getPerson(firstname: string, lastname: string, email: string, role: PersonRole): Promise<Person | null> {
        const query = {
            firstname,
            lastname,
            email,
    ...
    async checkPersonWithIdExists(id: string): Promise<boolean> {
        const result = await this.databaseConnection.getCollectionEntries(this.collectionName, { id });
        return result.length >= 1;
    }
    ...
    async getPersonsByRole(role: PersonRole): Promise<Person[] | null> {
        ...
        return this.databaseConnection.getCollectionEntries(this.collectionName, query);
    }

    async addPerson(person: Person): Promise<Person> {
        return this.databaseConnection.addObject(this.collectionName, person);
    }

    async deletePerson(id: string): Promise<void> {
        await this.databaseConnection.deleteCollectionEntry(this.collectionName, { id });
    }

    async updatePerson(person: Person): Promise<void> {
        await this.databaseConnection.updateCollectionEntry(this.collectionName, { id: person.id }, person);
    }
}
