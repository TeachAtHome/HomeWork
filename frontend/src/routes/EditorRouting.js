...
class EditorRouting extends Component {
  ...
  async getDocument() {
    const response = await fetch(
      '/api/storage/document/' + this.state.documentRefId
    );
    console.log(response);
    const body = await response.text();
    if (response.status >= 400) throw Error(body.message);
    var content = JSON.parse(body).content;
    content = this.state.studentView
      ? this.createStudentViewOnContent(content)
      : content;
    this.setState({
      content: content
    });
  ...
  createStudentViewOnContent(content) {
    ...
    var c = content.replace(
      new RegExp('Antwort', 'g'),
      '<span contenteditable="true" class="enabled"> <span class="ico fr-deletable" contenteditable="true">Antwort</span> </span>'
    );
